 ...
 *
 * Unlike the Ghana dashboard, no country is pre-selected: the country picker
 * opens on all 245 entries and validation requires a choice. Defaulting to GH
 * here would hand every non-Ghanaian supplier the Ghana field set and GHS as
 * the payout currency, which is how someone ends up expecting to be paid in
 * cedis. The only other divergence from the Ghana copy of this file.
 */
export function initialPayoutMethodForm({ countryCode = "", currency } = {}) {
  return {
    type: "BANK_TRANSFER",
    accountName: "",
    accountNumber: "",
